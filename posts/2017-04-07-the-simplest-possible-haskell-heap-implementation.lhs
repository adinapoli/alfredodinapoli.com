...
This implementation is shamelessly stolen from [Okasaki's book]()
...
See [this blog post](http://algs4.cs.princeton.edu/24pq/) for another excellent
...
>
> {-- Simplest-possible priority queue based on Leftist Heap
>
> Leftist Heap:
>
...
> This makes sense. When we decide which branch to go, if we know one branch is always shorter than the other,
> then we should take it. This is because shorter branch means potentially less nodes and the number
> of future possible operations intends to be smaller. In order to maintain this property,
>
> **each node has a rank, which indidates the length of the path between the node and the right most leaf.**
>
> --}
>
> --------------------------------------------------------------------------------
...
> 
> --------------------------------------------------------------------------------
> -- Make heap is straightforward: we compare the two ranks, and we preserve
